...
export class DwFitDialog extends LitElement {
  ...
  /**
   * Dispatches non bubbling `dw-fit-dialog-opened` or `dw-fit-dialog-closed` event based on `value`
   * On `opened` pushes instance into `window.openedDwFitDialogsInstances`. & lock scroll for previous opened dialog.
   * On `closed` removes last instance from `window.openedDwFitDialogsInstances` & unlock scroll for current opened dialog.
